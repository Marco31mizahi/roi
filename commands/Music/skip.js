  ...
  run: async ({ client, interaction, args, prefix }) => {
    ...
    if (!channel) {
      return interaction.followUp(
        `** ${emoji.ERROR} Önce Kanala Katılmanız Gerekiyor  **`
      );
    } else if (
      ...
      return interaction.followUp(`** ${emoji.ERROR} Oynatılan Parca Yok **`);
      ...
      queue.stop().catch((e) => {});
      return interaction.followUp(`** ${emoji.skip_track} Şarkı atlandı ! **`);
    } else {
      await queue.skip();
      interaction.followUp(`** ${emoji.skip_track} Şarkı Atlandı **`);
    }
  },
});
